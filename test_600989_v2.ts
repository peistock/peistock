...
async function test() {
  ...
  const indicators = calculateAllIndicators(data, 7333360000, 'shares');
  const last = indicators[indicators.length - 1];
  const prev = indicators[indicators.length - 2];
  
  console.log('=== 600989 宝丰能源 (真实流通股本) ===');
  ...
  console.log('前端 marketState:', frontend.marketState);
  console.log('前端 stateTitle:', frontend.stateTitle);
