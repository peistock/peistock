...
async function sendEmail(subject: string, htmlContent: string, attachments: { filename: string; path: string }[] = []) {
...
async function main() {
  ...
  const subject = `📊 股票信号扫描报告(完整版) - ${date} (${withSignals.length}只信号)`;
  
  let html = `
    <h2>📊 股票信号扫描报告(完整版) - ${date}</h2>
    <p><b>总股票数:</b> ${results.length} | <b>有信号:</b> ${withSignals.length}</p>
    <hr>
  `;
  
  if (withSignals.length > 0) {
    html += '<h3>⚠️ 关注以下股票：</h3><table border="1" cellpadding="8" style="border-collapse:collapse;">';
    html += '<tr style="background:#f0f0f0;"><th>代码</th><th>名称</th><th>收盘价</th><th>信号</th><th>BIAS</th><th>CRI</th><th>贪婪</th></tr>';
    
    for (const r of withSignals) {
      const signalColor = r.signals.some(s => s.includes('贪婪')) ? '#ff8800' : 
                         r.signals.some(s => s.includes('S')) ? '#03B172' : 
                         r.signals.some(s => s.includes('恐慌')) ? '#D946EF' : '#FF3435';
      html += `<tr>
        <td>${r.code}</td>
        <td>${r.name}</td>
        <td>¥${r.close}</td>
  ...
  html += '<hr><p style="color:#888;font-size:12px;">Peter趋势交易系统 完整版</p>';
  
  console.log('\n📧 正在发送邮件...');
  await sendEmail(subject, html, [{ filename: outputPath, path: path.resolve(process.cwd(), outputPath) }]);
