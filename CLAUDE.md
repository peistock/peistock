...
所有调 LLM 的入口都依赖 `~/family-mind` 提供 `mind.llm_client`,必须设 PYTHONPATH:

```bash
cd ~/rebel_research
PYTHONPATH=~/family-mind .venv/bin/python main.py          # 市场级日跑
PYTHONPATH=~/family-mind .venv/bin/python main_stock.py 600989  # 个股 A 股
PYTHONPATH=~/family-mind .venv/bin/python main_stock.py 01810   # 个股 HK
.venv/bin/python main_backtest.py --mock                   # 回测(不调 LLM)
.venv/bin/python panel.py                                  # Gradio 面板 http://localhost:7862
PYTHONPATH=~/family-mind uvicorn api_server:app --port 8000    # FastAPI（本地开发）
```

**生产部署**（JD Cloud 服务器）：
```bash
cd /opt/rebel_research
PYTHONPATH=/opt/family-mind:/opt/rebel_research nohup .venv/bin/uvicorn api_server:app --host 0.0.0.0 --port 8000 &
```
...
- **`~/family-mind`**:`mind.llm_client.LLMClient` —— Bull/Bear LLM 走它,`.env` 配置也在那里。本项目自己不维护 `.env`
...
- **searxng_proxy.py**(端口 8080):因 JD Cloud 服务器 GFW 屏蔽 Docker Hub/GitHub，无法部署 SearXNG Docker。改用轻量级 Python HTTP 代理，直接抓取百度/必应/搜狗结果，兼容 SearXNG `/search?q=&format=json` API 格式。
...
PYTHONPATH=~/family-mind .venv/bin/python -c "
import api_server
...
- **JD Cloud 无法部署 SearXNG Docker**:Docker Hub、GitHub 及国内镜像站均被 GFW 屏蔽。改用 `searxng_proxy.py`（Python HTTP 代理，8080 端口）直接抓取百度/必应/搜狗，返回 SearXNG 兼容 JSON。启动：`python3 searxng_proxy.py`
